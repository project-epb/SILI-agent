...
| `replyTimeout`  | `number` (ms) | `3600000`（1 小时） | 消息 → 操作映射的**内存**存活时间（热层）                 |
| `replyColdTimeout` | `number` (ms) | `604800000`（7 天） | 该映射在**数据库**中的存活时间（冷层），见下方说明     |
...
**引用（quote）机器人推送的某条消息**并发送以下内容，即可对该消息对应的 GitHub 资源操作。可引用的时限见下方「引用回复的两层时限」：
...
| 层 | 存放位置 | 时限 | 说明 |
| --- | --- | --- | --- |
| 热 | 内存 | `replyTimeout`（1 小时） | 同步命中，零开销 |
| 冷 | 数据库 | `replyColdTimeout`（7 天） | 内存未命中时回查，命中后重新载入内存 |

两层各自独立过期：条目从内存消失后，数据库里的行仍在，因此**重启机器人、或引用几天前的推送消息，快捷操作依然可用**。冷层过期的行在每次启动时清理一次，没有定时任务。

数据库不可用时，一切降级为纯内存行为（不影响推送，也不影响热层内的引用回复）。
...
- **引用回复**：广播时，插件把「消息 → 该事件支持的操作及对应 API 地址」的映射同时写入内存（`replyTimeout`）与数据库（`replyColdTimeout`）；当用户引用某条消息时据此定位资源并执行操作，内存未命中则回查数据库。详见上文「引用回复的两层时限」。
